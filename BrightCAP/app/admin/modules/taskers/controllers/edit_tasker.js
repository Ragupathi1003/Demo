angular.module('handyforall.taskers')
    .controller('addTaskerCtrl', addTaskerCtrl)
    .controller('DocumentModalInstanceCtrl', DocumentModalInstanceCtrl)
    .filter('checkTaskerDocs', checkTaskerDocs);

addTaskerCtrl.$inject = ['taskerAddServiceResolve', '$filter', '$state', 'TaskersService', '$scope', '$modal', 'toastr', 'CategoryServiceResolve', '$timeout', '$http', '$stateParams', 'NgMap', 'SettingsService', 'GeneralSettingsServiceResolve', '$rootScope', '$uibModal', 'TaskerDocumentResolve', 'AppService'];

function addTaskerCtrl(taskerAddServiceResolve, $filter, $state, TaskersService, $scope, $modal, toastr, CategoryServiceResolve, $timeout, $http, $stateParams, NgMap, SettingsService, GeneralSettingsServiceResolve, $rootScope, $uibModal, TaskerDocumentResolve, AppService) {
    ...
    if (angular.isDefined(TaskerDocumentResolve.response) && TaskerDocumentResolve.response.length > 0) {
        atsc.taskerdocument = TaskerDocumentResolve.response;
    }
    ...
    atsc.showEditForm = function (doc) {
        ...
        modalInstance.result.then(function (docdata) {
            if (angular.isDefined(docdata)) {
                ...
                data.doc = docdata;
                AppService.httpUploadRequest('/taskers/taskerDocumentEdit', data).then(function (response) {
                    if (response.data.status == 1) {
                        toastr.success('Document Updated Successfully');
                        if (response.data.result && response.data.result.length > 0) {
                            atsc.tasker.doc = response.data.result;
                        }
                    } else {
    ...
    atsc.deleteTaskerDocument = function (id) {
        var modalInstance = $modal.open({
            ...
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.deleteData = function () {
                    var index = atsc.tasker.doc.findIndex(function (e) { return e.id.toString() === id.toString() });
                    if (index !== -1) {
                        atsc.tasker.doc.splice(index, 1);
                    }
                    var data = {};
                    data.tasker = atsc.tasker._id;
                    data.id = id;
        ...
        modalInstance.result.then(function (docdata) {
            if (angular.isDefined(docdata)) {
                AppService.httpRequest('POST', '/admin/tasker/update-documents', docdata).then(function (response) {
                    if (response.status === 1) {
                        toastr.success('Document deleted successfully');
                        atsc.tasker.doc = response.result;
                    } else {
