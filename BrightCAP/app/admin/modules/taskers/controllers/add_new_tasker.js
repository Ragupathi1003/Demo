...
addNewTaskerCtrl.$inject = ['$filter', '$state', '$scope', '$modal', 'toastr', '$timeout', 'TaskersService', 'taskerAddServiceResolve', 'TaskerDocumentResolve', 'CategoryServiceResolve', 'NgMap', 'SettingsService', 'GeneralSettingsServiceResolve', 'Upload', 'AppService'];

function addNewTaskerCtrl($filter, $state, $scope, $modal, toastr, $timeout, TaskersService, taskerAddServiceResolve, TaskerDocumentResolve, CategoryServiceResolve, NgMap, SettingsService, GeneralSettingsServiceResolve, Upload, AppService) {
    ...
    antsc.taskerdocument = [];
    antsc.document = [];
    if (angular.isDefined(TaskerDocumentResolve.response) && TaskerDocumentResolve.response.length > 0) {
        antsc.taskerdocument = TaskerDocumentResolve.response;
    }
    antsc.uploadDocument = function (file, docs) {
        if (file) {
            Upload.upload({
                ...
                if (response.data.status == 1 || response.data.status == "1") {
                    var dataObj = {};
                    dataObj.name = docs.name;
                    dataObj.id = docs._id;
                    dataObj.file_type = response.data.document.file_type;
                    dataObj.path = response.data.document.path;
                    var index = antsc.document.findIndex(function (e) { return e.id.toString() === docs._id.toString() });
                    if (index !== -1) {
                        antsc.document[index] = dataObj;
                    } else {
                        antsc.document.push(dataObj);
                    }
                } else {
    ...
    antsc.savedocuments = function (valid) {
        if (valid) {
            if (angular.isDefined(antsc.document) && antsc.document.length > 0) {
                antsc.tasker.doc = antsc.document;
            }
            AppService.httpRequest('POST', '/taskers/addtasker', antsc.tasker).then(function (response) {
                var _id = "";
                _id = response._id;
                antsc.tasker._id = response._id;
                toastr.success('Tasker Added Successfully');
