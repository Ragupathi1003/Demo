...
registerCtrl.$inject = ['$scope', '$compile', '$rootScope', 'uiCalendarConfig', '$uibModal', 'CategoryserviceResolve', 'TaskerDocumentResolve', '$location', 'AuthenticationService', '$state', '$filter', 'toastr', '$cookieStore', '$stateParams', '$translate', 'accountService', 'NgMap', '$timeout', 'MainService', 'routes', 'settingsResolve', '$window', 'Upload'];

function registerCtrl($scope, $compile, $rootScope, uiCalendarConfig, $uibModal, CategoryserviceResolve, TaskerDocumentResolve, $location, AuthenticationService, $state, $filter, toastr, $cookieStore, $stateParams, $translate, accountService, NgMap, $timeout, MainService, routes, settingsResolve, $window, Upload) {
    var rgc = this;
    ...
    rgc.document = [];
    rgc.uploadDocument = function(file, docs) {
		console.log('file, docs', file, docs);
        if (file) {
            Upload.upload({
                url: '/site/tasker/taskerDocumentAdd',
                data: { file: file }
                ...
                if (response.data.status == 1) {
                    ...
                    var index = rgc.document.findIndex(function (e) { return e.id.toString() === docs._id.toString() });
                    if (index !== -1) {
                        rgc.document[index] = dataObj;
                    } else {
                        rgc.document.push(dataObj);
                    }
                } else {
    ...
    */
    ...
    rgc.TaskerRegister = function (valid, language) {
        ...
        if (valid > 0) {
            ...
            rgc.taskerData.doc = [];
			if (rgc.document && rgc.document.length > 0) {
				rgc.taskerData.doc = rgc.document;
			}
