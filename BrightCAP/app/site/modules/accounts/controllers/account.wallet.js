...
function accountWalletCtrl($scope, $rootScope, toastr, $window, $stateParams, MainService, routes, $uibModal, $translate, $state) {
    ...
    angular.forEach($scope.payment_method, function (value) {
        ...
        } else if (value.gateway_name == 'PayFast') {
            $scope.payfast_status = value.status;
        }
    });
    ...
    function WalletRechargeByPayfast(defaultCurrency) {
        $scope.walletMinAmt = ($scope.settings.wallet.amount.minimum * defaultCurrency.value).toFixed(2);
        $scope.walletWelAmt = ($scope.settings.wallet.amount.welcome * defaultCurrency.value).toFixed(2);
        $scope.walletMaxAmt = ($scope.settings.wallet.amount.maximum * defaultCurrency.value).toFixed(2);
        var data = {};
        data.amount = parseInt($scope.walletAmount);
        data.currencyvalue = defaultCurrency.value;
        ...
        if (data.amount && data.amount != 'NaN') {
            if (!(parseFloat(data.amount) >= $scope.walletMinAmt)) {
                ...
                MainService.getData(routes.walletRechargePayfast, { data: data, user: user }).then(function (response) {
                    
                    if (response.status == 1 && response.payment_mode == "PayFast") {
                        var form = '';
                        var url = '';
                        ...
                        angular.element.each(response, function (key, value) {
                            if (key != 'mode' && key != 'payment_mode' && key != 'status') {
                                form += '<input type="hidden" name="' + key + '" value="' + value + '">';
                            }
                        });
                        angular.element('<form action="' + url + '" method="POST">' + form + '</form>').appendTo('body').submit();
                    } else {
    ...
    angular.extend($scope, {
        ...
        WalletRechargeByPaypal: WalletRechargeByPaypal,
        WalletRechargeByPayfast : WalletRechargeByPayfast,
