module.exports = function (app, io) {
    ...
    router.payfastinsert = function (req, res) {
        db.GetOneDocument('paymentgateway', { status: { $ne: 0 }, alias: 'payfast' }, {}, {}, function (err, paymentgateway) {
            if (err) {
                ...
                var data = {
                    ...
                    "settings" : {
                        "mode" : "sandbox",
                        "live_passphrase" : "Zukov1989000",
                        "live_merchant_key" : "chbor042mjjtu",
                        "live_merchant_id" : "15484007",
                        "sandbox_passphrase" : "Ragupathi_12345",
                        "sandbox_merchant_key" : "66z2d7g1dv3lv",
                        "sandbox_merchant_id" : "10017993"
                    },
                    "status" : 1
                }
                db.InsertDocument('paymentgateway', data, (err, result) => {
                    if(err){
                        res.send({
                            "status": "0",
                            "message": res.__("Error, Please try Again Later.!")
                        ...
                        res.send({
                            "status": "1",
                            "message": res.__("Success, Successfull insert payfast data.!")
                        });
                    }
                })
            }else{
                res.send({
                    "status": "0",
                    "message": res.__("Error, Already inserted!")
                });
            }
        })
    }
