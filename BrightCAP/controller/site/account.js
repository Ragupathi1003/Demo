...
controller.getSearchTaskList = function getSearchTaskList(req, res) {
    ...
    //-----------06-04-2020-------------- payfast payment gateway
                    ...
                    if (err) {
                        ...
                        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
                            if (err || !currencies) {
                                ...
                                // var return_url = "https://eb06d9031cff.ngrok.io/site/account/payfastSuccess";
                                // var cancel_url = "https://eb06d9031cff.ngrok.io/site/account/payfastCancel";
                                // var notify_url = "https://eb06d9031cff.ngrok.io/site/account/payfastExecute";

                                var return_url = settings.site_url+"site/account/payfastSuccess";
                                var cancel_url = settings.site_url+"site/account/payfastCancel";
                                var notify_url = settings.site_url+"site/account/payfastExecute";


                                var item_name = task.booking_information.service_type;
                                if (paymentgateway.settings.mode == "sandbox") {
                                    var signatureData = `merchant_id=${encodeURIComponent(paymentgateway.settings.sandbox_merchant_id).replace(/[!'()*]/g, escape)}&merchant_key=${encodeURIComponent(paymentgateway.settings.sandbox_merchant_key).replace(/[!'()*]/g, escape)}&return_url=${encodeURIComponent(return_url).replace(/[!'()*]/g, escape)}&cancel_url=${encodeURIComponent(cancel_url).replace(/[!'()*]/g, escape)}&notify_url=${encodeURIComponent(notify_url).replace(/[!'()*]/g, escape)}&amount=${encodeURIComponent(request.avail_amount).replace(/[!'()*]/g, escape)}&item_name=${encodeURIComponent(item_name).replace(/%20/g, "+", escape)}&custom_str1=${encodeURIComponent(request.user).replace(/[!'()*]/g, escape)}&custom_str2=${encodeURIComponent(request.task).replace(/[!'()*]/g, escape)}&custom_str3=${encodeURIComponent(transaction._id).replace(/[!'()*]/g, escape)}&passphrase=${encodeURIComponent(paymentgateway.settings.sandbox_passphrase).replace(/[!'()*]/g, escape)}`;
                                } else {
                                    var signatureData = `merchant_id=${encodeURIComponent(paymentgateway.settings.live_merchant_id).replace(/[!'()*]/g, escape)}&merchant_key=${encodeURIComponent(paymentgateway.settings.live_merchant_key).replace(/[!'()*]/g, escape)}&return_url=${encodeURIComponent(return_url).replace(/[!'()*]/g, escape)}&cancel_url=${encodeURIComponent(cancel_url).replace(/[!'()*]/g, escape)}&notify_url=${encodeURIComponent(notify_url).replace(/[!'()*]/g, escape)}&amount=${encodeURIComponent(request.avail_amount).replace(/[!'()*]/g, escape)}&item_name=${encodeURIComponent(item_name).replace(/%20/g, "+", escape)}&custom_str1=${encodeURIComponent(request.user).replace(/[!'()*]/g, escape)}&custom_str2=${encodeURIComponent(request.task).replace(/[!'()*]/g, escape)}&custom_str3=${encodeURIComponent(transaction._id).replace(/[!'()*]/g, escape)}&passphrase=${encodeURIComponent(paymentgateway.settings.live_passphrase).replace(/[!'()*]/g, escape)}`;
                                }
                                signatureData = signatureData.replace(/(\r\n|\n|\t|\r)/gm, "");

                                var signature = crypto.createHash('md5').update(signatureData).digest('hex');
                                if (paymentgateway.settings.mode == "sandbox") {
                                var data = {
                                    'merchant_id': paymentgateway.settings.sandbox_merchant_id,
                                    'merchant_key': paymentgateway.settings.sandbox_merchant_key,
                                    'item_name': item_name,
                                    'amount': request.avail_amount,
                                    'return_url': return_url,
                                    'cancel_url': cancel_url,
                                    'notify_url': notify_url,
                                    'signature': signature,
                                    'custom_str1': request.user,
                                    'custom_str2': request.task,
                                    'custom_str3': transaction._id,
                                    'mode': paymentgateway.settings.mode,
                                ...
                                var data = {
                                    'merchant_id': paymentgateway.settings.live_merchant_id,
                                    'merchant_key': paymentgateway.settings.live_merchant_key,
                                    'item_name': item_name,
                                    'amount': request.avail_amount,
                                    'return_url': return_url,
                                    'cancel_url': cancel_url,
    ...
    controller.payfastExecute = function payfastExecute(req, res) {
        ...
        if (payment_status == 'COMPLETE') {
            ...
            var request = {};
            request.user = req.body.custom_str1;
            request.task = req.body.custom_str2;
            request.transaction = req.body.custom_str3;
            request.amount_gross = req.body.amount_gross;
            ...
            ], function (err, paymentgateway, transaction) {
                if (err) {
                    console.log(err);
                }
                else {
                    ...
                    if (user == request.user && task == request.task && amount == request.amount_gross) {
                        taskLibrary.taskPayment({ 'transaction': transaction._id, 'gateway_response': payment_status, 'task': task }, function (err, response) {
                            if (err || !response) {
                                res.redirect("/payment-failed/" + req.query.task);
                            }
    ...
    controller.payfastSuccess = function payfastSuccess(req, res) {
        res.redirect("/payment-success");
    }

    controller.payfastCancel = function payfastCancel(req, res) {
        res.redirect("/payment-failed");
    }

    controller.walletPayfastPayment = function walletPayfastPayment(req, res) {
        ...
        req.checkBody('data.amount', 'Invalid Amount').notEmpty();
        req.checkBody('user', 'Invalid User Id').notEmpty();
            ...
            if (err || !user) {
                ...
                async.waterfall([
                    ...
                    if (err) {
                        ...
                        // var return_url = "https://cd896a7d979f.ngrok.io/site/account/walletPayfastSuccess";
                        // var cancel_url = "https://cd896a7d979f.ngrok.io/site/account/walletPayfastCancel";
                        // var notify_url = "https://cd896a7d979f.ngrok.io/site/account/walletpayfastExecute";

                        var return_url = settings.site_url+"site/account/walletPayfastSuccess";
                        var cancel_url = settings.site_url+"site/account/walletPayfastCancel";
                        var notify_url = settings.site_url+"site/account/walletpayfastExecute";


                        var item_name = "Wallet Recharge";
                        var totalamount = (request.amount).toFixed(2);
                        console.log(typeof request.amount,totalamount);
                        
                        if (paymentgateway.settings.mode == "sandbox") {

                         var signatureData = `merchant_id=${encodeURIComponent(paymentgateway.settings.sandbox_merchant_id).replace(/[!'()*]/g, escape)}&merchant_key=${ encodeURIComponent(paymentgateway.settings.sandbox_merchant_key).replace(/[!'()*]/g, escape)}&return_url=${ encodeURIComponent(return_url).replace(/[!'()*]/g, escape)}&cancel_url=${ encodeURIComponent(cancel_url).replace(/[!'()*]/g, escape)}&notify_url=${ encodeURIComponent(notify_url).replace(/[!'()*]/g, escape)}&amount=${encodeURIComponent(totalamount).replace(/[!'()*]/g, escape)}&item_name=${ encodeURIComponent(item_name).replace(/%20/g, "+", escape)}&custom_str1=${ encodeURIComponent(transaction._id).replace(/[!'()*]/g, escape)}&custom_str2=${ encodeURIComponent(request.user).replace(/[!'()*]/g, escape)}&passphrase=${ encodeURIComponent(paymentgateway.settings.sandbox_passphrase).replace(/[!'()*]/g, escape)}`;
                        }else{
                            var signatureData = `merchant_id=${encodeURIComponent(paymentgateway.settings.live_merchant_id).replace(/[!'()*]/g, escape)}&merchant_key=${ encodeURIComponent(paymentgateway.settings.live_merchant_key).replace(/[!'()*]/g, escape)}&return_url=${ encodeURIComponent(return_url).replace(/[!'()*]/g, escape)}&cancel_url=${ encodeURIComponent(cancel_url).replace(/[!'()*]/g, escape)}&notify_url=${ encodeURIComponent(notify_url).replace(/[!'()*]/g, escape)}&amount=${encodeURIComponent(totalamount).replace(/[!'()*]/g, escape)}&item_name=${ encodeURIComponent(item_name).replace(/%20/g, "+", escape)}&custom_str1=${ encodeURIComponent(transaction._id).replace(/[!'()*]/g, escape)}&custom_str2=${ encodeURIComponent(request.user).replace(/[!'()*]/g, escape)}&passphrase=${ encodeURIComponent(paymentgateway.settings.live_passphrase).replace(/[!'()*]/g, escape)}`;
                        }
                        signatureData = signatureData.replace(/(\r\n|\n|\t|\r)/gm, "");
                        ...
                        if (paymentgateway.settings.mode == "sandbox") {
                            var data = {
                                ...
                                'notify_url': notify_url,
                                //'signature': signature,
                                'custom_str1': transaction._id,
                                'custom_str2': request.user,
                                'mode': paymentgateway.settings.mode,
                                'status': paymentgateway.status,
                                'payment_mode': 'PayFast',
                            };
    ...
    controller.walletPayfastExecute = function walletPayfastExecute(req, res) {
        ...
        var request = {};
        request.transaction = req.body.custom_str1;
        request.user = req.body.custom_str2;
        request.amount_gross = req.body.amount_gross;
            ...
            if (err) {
                ...
                async.waterfall([
                    ...
                    if (err) {
                        ...
                        if (payment_status == 'COMPLETE') {
                            console.log('gateway_response amount', request.amount_gross);
                            if (amount == request.amount_gross) {
                                userLibrary.walletRecharge({ 'user': request.user, 'transaction': request.transaction, 'gateway_response': payment_status, 'task': task }, function (err, response) {
                                    if (err || !response) {
                                        res.redirect("/payment-failed" + request.user);
                                    }
                                });
    ...
    controller.walletPayfastSuccess = function walletPayfastSuccess(req, res) {
        res.redirect("/wallet-success");
    }

    controller.walletPayfastCancel = function walletPayfastCancel(req, res) {        
        res.redirect("/payment-failed");
    }
