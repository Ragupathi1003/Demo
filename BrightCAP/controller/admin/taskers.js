    ...
    router.updatetaskerstatus = function (req, res) {
        ...
        db.GetDocument('tasker', { _id: req.body.id }, {}, {}, function (err, tasker) {
            if (err) {
                ...
                db.UpdateDocument('tasker', { _id: req.body.id }, { status: status, doc_status }, function (err, updatedata) {
                    if (err) {
                        res.send(err);
                    } else {
                        var message = '';
...
*/
    ...
    router.activeDocumentLists = function(req, res) {
        var data = {};
        data.status = 0;
        ...
        db.GetAggregation('tasker_documents', aggregationQuery, function(error, documentsData) {
            if (error) {
                data.response = error.message;
                res.send(data);
            } else {
                data.status = 1;
                data.response = documentsData;
                res.send(data);
            }
        });
    }
