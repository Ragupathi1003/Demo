module.exports = function(io, i18n) {
            ...
            if (err) {
              ...
              db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function(err, currencies) {
                if (err || !currencies) {
                  ...
                  data.response.document_upload_status = (settings.settings.document_upload) ? settings.settings.document_upload.status : 0;;
    ...
    { day: res.__("Saturday"), slot: JSON.parse(JSON.stringify( availableTimingSlotList )), selected: false, wholeday: false}];
      ...
      db.GetOneDocument('settings', {'alias': 'general'}, {}, {}, function (err, settings) {
        ...
        data.document_upload_status = (settings.settings.document_upload) ? settings.settings.document_upload.status : 0;
            ...
            db.GetOneDocument('settings', {'alias': 'general'}, {}, {}, function (err, settings) {
              if (err || !settings) {
                res.send({
                  ...
                  res.send({
                    "status": "0", "documents": docs, "upload_status": (settings.settings.document_upload) ? settings.settings.document_upload.status : 0
                  });
              }
            });
        }
